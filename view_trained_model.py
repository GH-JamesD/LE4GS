...
def viewing(dataset, opt, pipe, checkpoint, gaussian_dim, time_duration, rot_4d, force_sh_3d):
    ...
    while i < 10:
        ...
        while network_gui.conn != None:
            try:
                ...
                custom_cam, _, pipe.convert_SHs_python, pipe.compute_cov3D_python, keep_alive, scaling_modifer, show_legs_image = network_gui.receive()
                if custom_cam != None:
                    render_pkg = render(custom_cam, gaussians, pipe, background, opt, scaling_modifer)
                    if show_legs_image:
                        net_image = render_pkg['language_feature_image']
                        # print(np.all(np.ravel(net_image.cpu().detach().numpy())==0))
                    else:
