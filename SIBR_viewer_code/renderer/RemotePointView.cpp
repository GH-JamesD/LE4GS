...
// CUSTOM CODE
constexpr char* jTimestamp = "timestamp";
constexpr char* jShowLEGSImage = "show_legs_image";
			...
			{
				{
					...
					// CUSTOM CODE
					sendData[jTimestamp] = _timestamp;
					sendData[jShowLEGSImage] = _showLEGSImage;
...
void sibr::RemotePointView::onGUI()
{
	...
	if (ImGui::Begin(guiName.c_str())) 
	{
		...
		// CUSTOM CODE
		ImGui::SliderFloat("Timestamp", &_timestamp, 0.0f, 1.0f);
		ImGui::Checkbox("Show LEGS Image", &_showLEGSImage);
	}
	ImGui::End();
}
